...
int main (){
	/int serviceLane;
	 int i,j,k,n,t,numberOfRows,numberOfColumns;
	 int cases[i,j];
	
	n = number of width
	t = number of test cases
	i,j = entry and exit indices repectively
	 ...
	 for(1<=n<=1000)
	 for(0<=i<j<n)
	 for(2<=j-i+1<=min(n,1000))
	 for(1<=width[k]<=3;0<=k<=n)
	 {
	 	printf("%d"\n)
	 }
	return 0; 
...
int* serviceLane(int n, int cases_rows, int cases_columns, int** cases, int* result_count) {
...
int main()
{
   ...
   int* width = malloc(n * sizeof(int));
    ...
    int result_count;
    int* result = serviceLane(n, t, 2, cases, &result_count);
